          ...
          <div className={styles.modal} onClick={e => e.stopPropagation()} style={{ maxWidth: '400px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
              ...
              <div style={{ borderTop: '1px solid var(--color-border)', paddingTop: '0.75rem' }}>
                {!showFriendLink ? (
                  <button
                    onClick={() => { setShowFriendLink(true); setFriendLinkSearch(''); }}
                    style={{ width: '100%', padding: '0.5rem', border: '1px dashed var(--color-border)', borderRadius: 'var(--radius-md)', background: 'none', color: 'var(--color-accent)', fontSize: '0.82rem', fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}
                  >
                    👤 Link to Friend
                  </button>
                ) : (
                  <div>
                    <div style={{ fontSize: '0.72rem', fontWeight: 600, color: 'var(--color-text-secondary)', textTransform: 'uppercase', marginBottom: '0.25rem' }}>Link to Friend</div>
                    <input
                      type="text"
                      value={friendLinkSearch}
                      onChange={e => setFriendLinkSearch(e.target.value)}
                      placeholder="Search friends by name..."
                      autoFocus
                      style={{ width: '100%', padding: '0.45rem 0.65rem', border: '1px solid var(--color-border)', borderRadius: 'var(--radius-md)', fontSize: '0.85rem', fontFamily: 'inherit', boxSizing: 'border-box', marginBottom: '0.35rem' }}
                    />
                    <div style={{ maxHeight: '150px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '0.2rem' }}>
                      {friends
                        .filter(f => {
                          if (!friendLinkSearch.trim()) return true;
                          const q = friendLinkSearch.toLowerCase();
                        ...
                        .map(f => (
                          <button
                            ...
                            onClick={() => {
                              ...
                              setShowFriendLink(false);
                              setFriendLinkSearch('');
                            }}
                            style={{
                              display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.4rem 0.6rem',
                              border: '1px solid var(--color-border)', borderRadius: 'var(--radius-md)', background: 'var(--color-surface)',
                              cursor: 'pointer', fontFamily: 'inherit', textAlign: 'left', fontSize: '0.82rem',
                            }}
                            onMouseEnter={e => e.currentTarget.style.background = 'var(--color-accent-light)'}
                            onMouseLeave={e => e.currentTarget.style.background = 'var(--color-surface)'}
                          >
                            <div style={{ width: '28px', height: '28px', borderRadius: '50%', background: 'var(--color-accent-light)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.72rem', fontWeight: 700, color: 'var(--color-accent)', flexShrink: 0 }}>
                              {(f.name || '?')[0].toUpperCase()}
                            </div>
                            <div style={{ flex: 1, minWidth: 0 }}>
                              <div style={{ fontWeight: 600, color: 'var(--color-text)' }}>{f.name || 'Unknown'}</div>
                      ...
                      {friends.length === 0 && <div style={{ padding: '0.5rem', fontSize: '0.82rem', color: 'var(--color-text-muted)', textAlign: 'center' }}>No friends added yet</div>}
                    </div>
                    <button onClick={() => setShowFriendLink(false)} style={{ marginTop: '0.35rem', padding: '0.3rem 0.6rem', border: '1px solid var(--color-border)', borderRadius: 'var(--radius-md)', background: 'var(--color-surface)', fontSize: '0.75rem', cursor: 'pointer', fontFamily: 'inherit', color: 'var(--color-text-muted)' }}>Cancel</button>
                  </div>
                )}
                {editMemberFields.linkedFriendId && (
                  <div style={{ marginTop: '0.35rem', fontSize: '0.75rem', color: 'var(--color-success)', fontWeight: 600 }}>
                    ✓ Linked to friend — name, email, and phone updated
                  </div>
                )}
              </div>
