...
// The block is a year-long chart now; the shape of the chart itself is pinned
// in sportsYearTimeline.test.js. What matters here is that the words beside a
// bar still say the phase, the date it runs to and how long that leaves.
describe('buildSeasonBlock', () => {
  const day = 86400000;
  const iso = (d) => new Date(Date.now() + d * day).toISOString();

  it('states the phase and the days left, with no "In season" preamble', () => {
